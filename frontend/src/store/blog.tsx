...
const useBlogStore = create<BlogStore>((set) => ({
  ...
  likedBlogs: async () => {
    try {
      const response = await axiosInstance.get("/v2/blog/liked-blogs");
  ...
  getViews: async () => {
    try {
      const response = await axiosInstance.get(`/v2/blog/my-views`);
      set({ views: response.data.views });
    } catch (error: any) {
      toast.error("Failed to fetch views", { duration: 3000 });
      console.error(error);
    }
  },
  getComments: async () => {
